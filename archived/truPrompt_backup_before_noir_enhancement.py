...
class FilmNoirAdventure:
    def __init__(self):
        ...
        self.current_room = self.player_data.get("location", "office")
        self.inventory = self.player_data.get("inventory", [])
        self.game_over = False
        self.won = False

    def _load_game_data(self):
        ENCODED_GAME_DATA = ''
        try:
            game_data_json = base64.b64decode(ENCODED_GAME_DATA).decode('utf-8')
            data = json.loads(game_data_json)
            self.rooms = data["rooms"]
            self.items = data["items"]
            self.player_data = data["player"]
        except (json.JSONDecodeError, KeyError, Exception) as e:
            print(f"Critical error loading game data: {e}")
            self.rooms = {"error": {"desc": "The story is lost to the void..."}}
            self.items = {}
            self.player_data = {"location": "error", "inventory": []}
            self.game_over = True

    def _wrap_text(self, text):
        return "\\n".join(textwrap.wrap(text, width=70))

    def _handle_go(self, target, room):
        if target in room.get("exits", {}):
            self.current_room = room["exits"][target]
            print(f"\\nYou go to the {target}...")
        else:
            print("You can't go that way, pal.")

    def _handle_take(self, target, room):
        if target in room.get("items", []) and self.items.get(target, {}).get("takeable"):
            self.inventory.append(target)
            room["items"].remove(target)
            print(f"You take the {target}.")
        else:
            print("That's not here or you can't take it.")

    def _handle_talk(self, target, room):
        if target in room.get("npcs", {}):
            print(self._wrap_text(room["npcs"][target]))
        else:
            print(f"There's no one named {target} here to talk to.")

    def _handle_look(self, target, room):
        # Check if it's an item in the room
        if target in room.get("items", []):
            item_desc = self.items.get(target, {}).get("desc", "You see nothing special about it.")
            print(self._wrap_text(f"You examine the {target}. {item_desc}"))
        # Check if it's an NPC
        elif target in room.get("npcs", {}):
            print(self._wrap_text(f"You study {target} carefully. {room['npcs'][target]}"))
        # Check for special room features
        elif target in self._get_room_features(room):
            print(self._wrap_text(self._get_room_feature_desc(room, target)))
        else:
            print(f"You don't see a {target} here to look at.")

    def _get_room_features(self, room):
        features = []
        desc = room.get("desc", "").lower()
        ...
        if "desk" in desc:
            features.append("desk")
        
        # Bar features
        if "bartender" in desc:
            features.append("bartender")
        if "bar" in desc:
            features.append("bar")
        if "thug" in desc:
            features.append("thug")
            
        # Pawn shop features
        if "counter" in desc:
            features.append("counter")
        if "gamer" in desc:
            features.append("gamer")
            
        # Dock features
        if "water" in desc or "pier" in desc:
            features.append("water")
        if "ships" in desc:
            features.append("ships")
        ...
        if "circuits" in desc:
            features.append("circuits")
            
        return features

    def _get_room_feature_desc(self, room, feature):
        feature_descs = {
            "poster": "The recruitment poster shows a stern-faced officer with the words 'Start the Fires' in bold letters. There's something written in small print at the bottom: 'The truth burns brightest in the darkest places.'",
            "crack": "The crack in the wall runs from floor to ceiling. Behind it, you can see what looks like a small hidden compartment. It's empty now, but someone clearly used it to hide something.",
            "desk": "The desk is covered in dust and old case files. The whiskey bottle sits next to a half-empty coffee cup and a stack of unread reports. There's a drawer that's slightly ajar.",
            "bartender": "The bartender has seen better days. His eyes are bloodshot and his hands shake slightly as he polishes a glass. He looks like he knows more than he's letting on.",
            "bar": "The bar itself is made of dark mahogany, scarred with years of use. There are initials carved into the wood, and the brass rail is worn smooth from countless elbows.",
            "thug": "The tired, fire-eyed thug slouches in the corner, muttering to himself. His clothes are dirty and torn, and he has the look of someone who's been running for a long time.",
            "counter": "The pawn shop counter is cluttered with various trinkets and oddities. Behind it, you can see shelves filled with mysterious objects, each with its own story.",
            "gamer": "The twitchy gamer hunches over his handheld device, completely absorbed in whatever game he's playing. He barely notices you, but occasionally glances up with nervous eyes.",
            "water": "The dark water laps against the rotting pylons. You can see the reflection of neon signs in the oily surface, and something large moves just beneath the surface.",
            "ships": "The ships creak in the night, their hulls groaning against the dock. They look like they've seen better days, but there's something ominous about their silhouettes.",
            "crates": "The crates are stacked to the ceiling, each one marked with cryptic symbols and numbers. Some are open, revealing strange contents that don't look like normal cargo.",
            "ledger": "The heavy leather-bound ledger is filled with cryptic entries, shipping manifests, and what appear to be coded messages. The handwriting is neat but the content is deliberately obscure.",
            "sign": "The glowing sign pulses with an otherworldly energy. The words 'Grok the Truth, or Start the Fires' seem to shift and change as you look at them. It's definitely not normal technology.",
            "oracle": "The holographic construct shimmers into existence. Its form is constantly shifting, never quite solid, and its voice seems to come from everywhere and nowhere at once.",
            "circuits": "The exposed circuits pulse with an eerie blue light. They seem to be running calculations that are far beyond normal computer processing, and the patterns are hypnotic to watch."
        }
        return feature_descs.get(feature, f"You examine the {feature}, but see nothing unusual about it.")

    def _handle_use(self, target_item, on_object):
        if target_item == "key" and on_object == "lockbox" and "key" in self.inventory and "lockbox" in self.rooms["pawn"]["items"]:
            print(self._wrap_text("The brass key turns smoothly in the lock. The lockbox clicks open. Inside is a slip of paper with an address: 'XAI 7, Pier 4.' This must be where the real books are kept."))
            self.inventory.remove("key")
        elif target_item == "grok_orb" and on_object == "ledger" and "grok_orb" in self.inventory and "ledger" in self.rooms["warehouse"]["items"]:
            print(self._wrap_text("You hold the orb to the ledger. It glows, and the cryptic entries rearrange into a coherent story of smuggling and betrayal. You've grokked the truth."))
            self.won = True
            self.game_over = True
        else:
            print("You can't use that like that.")

    def play(self):
        print("\\n" + "="*70)
        print("EASTER EGG: FILM NOIR MYSTERY - 'THE CASE OF THE GLOWING ORB'")
        print("="*70)
        print("Commands: 'go <exit>', 'take <item>', 'talk to <person>',")
        print("          'look at <item/feature>', 'examine <item/feature>',")
        print("          'use <item> on <object>', 'inventory', 'quit'.")
        print("Good luck, gumshoe.\\n")
        
        while not self.game_over:
            room = self.rooms.get(self.current_room, {"desc": "You are lost."})
            print("---")
            print(self._wrap_text(room.get("desc", "")))
            
            if room.get("items"):
                print(f"Items here: {', '.join(room['items'])}")
            if room.get("npcs"):
                print(f"People here: {', '.join(room['npcs'].keys())}")
            
            # Show interactive features
            features = self._get_room_features(room)
            if features:
                print(f"Interactive features: {', '.join(features)}")
            ...
            if command == "quit":
                ...
                self._handle_go(" ".join(words[1:]), room)
            elif action == "take" and len(words) > 1:
                self._handle_take(" ".join(words[1:]), room)
            elif action == "talk" and len(words) > 2 and words[1] == "to":
                self._handle_talk(" ".join(words[2:]), room)
            elif (action == "look" and len(words) > 2 and words[1] == "at") or action == "examine":
                if action == "look":
                    target = " ".join(words[2:])
                else:
                    target = " ".join(words[1:])
                self._handle_look(target, room)
            elif action == "use" and "on" in words and len(words) > 3:
                parts = command.split(" on ")
                item_to_use = parts[0][4:].strip()
                object_to_use_on = parts[1].strip()
                self._handle_use(item_to_use, object_to_use_on)
                ...
                print("I don't understand that, pal. Keep it simple.")

        print("\\n" + "="*70)
        if self.won:
            print("YOU WIN! You walk out of the warehouse as sirens fill the night air. You grokked the truth. Good work, gumshoe.")
        else:
            print("GAME OVER. The city always wins. The rain washes the streets clean, but it'll never wash away the darkness.")
        print("="*70 + "\\n")
...
try:
    ...
    print(f"A mysterious error occurred in the shadows: {e}")
""", globals())
            except Exception as e:
